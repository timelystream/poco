...
#include "Poco/CompressedFileChannel.h"
...
const std::string CompressedFileChannel::PROP_PATH         = "path";
const std::string CompressedFileChannel::PROP_ROTATION     = "rotation";
const std::string CompressedFileChannel::PROP_ARCHIVE      = "archive";
const std::string CompressedFileChannel::PROP_TIMES        = "times";
const std::string CompressedFileChannel::PROP_COMPRESS     = "compress";
const std::string CompressedFileChannel::PROP_PURGEAGE     = "purgeAge";
const std::string CompressedFileChannel::PROP_PURGECOUNT   = "purgeCount";
const std::string CompressedFileChannel::PROP_FLUSH        = "flush";
const std::string CompressedFileChannel::PROP_ROTATEONOPEN = "rotateOnOpen";

CompressedFileChannel::CompressedFileChannel(): 
	_times("utc"),
	_compress(false),
	_flush(true),
	_rotateOnOpen(false),
...
CompressedFileChannel::CompressedFileChannel(const std::string& path):
	_path(path + ".lz4"),
	_times("utc"),
	_compress(false),
	_flush(true),
...
void CompressedFileChannel::open()
{
...
void CompressedFileChannel::close()
{
	FastMutex::ScopedLock lock(_mutex);
...
void CompressedFileChannel::log(const Message& msg)
{
	FastMutex::ScopedLock lock(_mutex);
	...
	if (_pRotateStrategy && _pArchiveStrategy && _pRotateStrategy->mustRotate(_pFile))
	{
		...
		{
			_pFile = new CompressedLogFile(_path);
		}
...
void CompressedFileChannel::setProperty(const std::string& name, const std::string& value)
{
	FastMutex::ScopedLock lock(_mutex);
...
std::string CompressedFileChannel::getProperty(const std::string& name) const
{
	if (name == PROP_TIMES)
		return _times;
	else if (name == PROP_PATH)
...
Timestamp CompressedFileChannel::creationDate() const
{
	if (_pFile)
...
UInt64 CompressedFileChannel::size() const
{
	if (_pFile)
		return _pFile->size();
	else
...
const std::string& CompressedFileChannel::path() const
{
	return _path;
}


void CompressedFileChannel::setRotation(const std::string& rotation)
{
...
void CompressedFileChannel::setArchive(const std::string& archive)
{
...
void CompressedFileChannel::setCompress(const std::string& compress)
{
...
void CompressedFileChannel::setPurgeAge(const std::string& age)
{
...
void CompressedFileChannel::setPurgeCount(const std::string& count)
{
...
void CompressedFileChannel::setFlush(const std::string& flush)
{
	_flush = icompare(flush, "true") == 0;
}


void CompressedFileChannel::setRotateOnOpen(const std::string& rotateOnOpen)
{
	_rotateOnOpen = icompare(rotateOnOpen, "true") == 0;
}


void CompressedFileChannel::purge()
{
	if (_pPurgeStrategy)
	{
		try
...
void CompressedFileChannel::unsafeOpen()
{
	if (!_pFile)
	{
		_pFile = new CompressedLogFile(_path);
		if (_rotateOnOpen && _pFile->size() > 0)
		{
			try
			{
				_pFile = _pArchiveStrategy->archive(_pFile, true);
				purge();
			}
			catch (...)
			{
...
bool CompressedFileChannel::setNoPurge(const std::string& value)
{
...
int CompressedFileChannel::extractDigit(const std::string& value, std::string::const_iterator* nextToDigit) const
{
...
void CompressedFileChannel::setPurgeStrategy(PurgeStrategy* strategy)
{
	delete _pPurgeStrategy;
...
Timespan::TimeDiff CompressedFileChannel::extractFactor(const std::string& value, std::string::const_iterator start) const
{
